...
for (i in c(1:nrow(lang_codes))) {
  ...
  final_voiced_obstruents_plot <- ggplot(data.frame(x=artificial$final_voiced_obstruents), aes(x, color="red",fill="red",alpha=.5)) +
...
save(final_voiced_obstruents_plots,file = "Data/plots/fvo_plots.Rdata")
```

```{r den-plots}
percentiles <- data.frame(lang_codes$code, final_voiced_obstruents)
...
save(final_voiced_percentile_density, file = "Data/plots/final_voiced_percentile_density.Rdata")
