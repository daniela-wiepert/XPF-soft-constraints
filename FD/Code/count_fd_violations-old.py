    ...
    for lang_code in lang_codes:
        ...
        for roots, dirs, files in os.walk('Data/artificial_baselines/' + lang_code):
            ...
            for f in files:
                ...
                percent = sum(map(voiced_final,baseline)) /baseline_len
            
                if not os.path.exists('Data/distributions/'):
                    os.makedirs('Data/distributions/')

                if not os.path.exists('Data/distributions/' + lang_code + '_artificial.csv'):
                    with open('Data/distributions/' + lang_code + '_artificial.csv', 'a+') as fout:
                        write = csv.writer(fout)
                        write.writerow(['final_voiced_obstruents'])

                with open('Data/distributions/' + lang_code + '_artificial.csv', 'a+') as fout:
                    write = csv.writer(fout)
                    write.writerow([percent])        
